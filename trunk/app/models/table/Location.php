...
class Location extends ITechTable
{
  ...
  */
  ...
  public static function getFieldName($suffix = 'id', $tier, $num_tiers = 4) {
        ...
        switch($num_tiers) {
           case 2:
             if ( $tier == 2) $field_name = 'city_'.$suffix;
             break;
        	 case 3:
             if ( $tier == 2) $field_name = 'district_'.$suffix;
             if ( $tier == 3) $field_name = 'city_'.$suffix;
             break;
           case 4:
             if ( $tier == 2) $field_name = 'district_'.$suffix;
             if ( $tier == 3) $field_name = 'region_c_'.$suffix;
             if ( $tier == 4) $field_name = 'city_'.$suffix;
             break;
           }
  ...
  public static function getCityInfo($loc_id, $num_tiers) {
  	$output = array(0=>null,1=>null,2=>null,3=>null);
  	if ( $loc_id ) {
  		$locations = self::getAll();
  ...
  public static function subquery($num_locs, $where_tier = false, $where_location_id = false, $include_ids = false) {
     ...
     for($l = $num_locs; $l > 0; $l--) {
       ...
       if ($include_ids ) $select_cols1 []= 'l'.$tier.'.id as '.$field_name[$l].'id';
     	 if ( $l < $num_locs) {
	       $joins1 []= " JOIN location l$tier ON l".($tier+1).".parent_id = l$tier.id  AND l$tier.tier = $tier ".($where_location_id && $where_tier == $tier?" AND l$tier.id IN (".implode(',',$where_location_id).')':'');
         $select_cols2 []= 'l'.$tier.'.location_name as '.$field_name[$l].'name';
         if ($include_ids ) $select_cols2 []= 'l'.$tier.'.id as '.$field_name[$l].'id';
         if ( $l < $num_locs-1) {
	         $joins2 []= " JOIN location l$tier ON l".($tier+1).".parent_id = l$tier.id  AND l$tier.tier = $tier ".($where_location_id && $where_tier == $tier?" AND l$tier.id IN (".implode(',',$where_location_id).')':'');
	       } 
     	 }
     }
