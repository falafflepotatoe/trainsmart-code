   ...
   public function countryRegionAction()
    ...
    //  $this->viewAssignEscaped('criteria',$criteria);
      ...
      if ( ($tier == 1) OR ($location_id && ($locations[$location_id]['tier'] + 1 == $tier))) {
	      ...
	      $editTable->fields  = array('location_name' => ($tier==3?$this->tr('Region C (Local Region)') : ($tier ==2 ? @$this->tr('Region B (Health District)') : @$this->tr('Region A (Province)') )));
	      $editTable->label   = ($tier==3?$this->tr('Region C (Local Region)') : ($tier ==2 ? @$this->tr('Region B (Health District)') : @$this->tr('Region A (Province)') ));
	      $editTable->dependencies = array(
	        'parent_id' => 'self',
